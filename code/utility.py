...
class checkpoint():
    def __init__(self, args):
        ...
        if args.demo:
            _make_dir(self.dir + '/' + args.testset)
        else:
    ...
    def save_results(self, filename, save_list, scale):
        filename = '{}/{}.format(self.dir, filename)'
        postfix = ('SR', 'LR', 'HR')
    ...
    def save_results_demo(self, filename, save_list):
        filename = '{}/{}/{}'.format(self.dir, self.args.testset, filename)
        postfix = ('SR', 'LR', 'HR')
        for v, p in zip(save_list, postfix):
            normalized = v[0].data.mul(255 / self.args.rgb_range)
            ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
